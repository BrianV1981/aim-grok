"""TDD: vessel-prefixed session names (soul API; aim-grok default vessel=grok)."""
import re
import subprocess
import sys
import time
...
def test_reincarnation_legacy_wrap():
    ...
    assert is_reincarnation_session(name, {})
...
def test_tmux_e2e_session_visible_under_vessel_prefix():
    name = build_agent_session_name(
        "testrole", "/tmp/project", {"AIM_VESSEL_CLI": "grok"}, timestamp=int(time.time())
    )
    assert re.fullmatch(r"grok_testrole_project_[0-9]+", name)
    subprocess.run(["tmux", "kill-session", "-t", name], capture_output=True)
    try:
        r = subprocess.run(
            ["tmux", "new-session", "-d", "-s", name, "sleep", "20"],
            capture_output=True,
            text=True,
            timeout=10,
        )
