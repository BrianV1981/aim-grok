...
**Epic:** Reincarnation / memory overhaul — **agent-proof handoff** separated from **nightly wiki+embed** and **nightly blackbox**.

This is a **major architectural shift**. The current reincarnate pipeline is too fragile, produces false greens, and depends on agents not being stupid (wrong tickets, wrong sessions, dual writers, spin-up ceremonies).

### Operator direction (confirmed)

| System | Trigger | Responsibility |
|--------|---------|----------------|
| **A. Handoff / reincarnation** | On demand (CLI / optional hook) | Continuity packet only — content-gated, no wiki/vault/spawn required |
| **B. Wiki + memory** | **Nightly cron** | By conversation root (cwd): **Flight Recorder clean** → wiki → **embed** → **save to memory DB** |
| **C. Blackbox** | **Nightly cron** | Seal **every** session raw — not via handoff process |

- **No agent spin-up** as part of handoff by default.
- **Codex** on hold indefinitely (adapters later).
- Flagship build order: **grok → agy → opencode**.
- Local plan copies: `docs/PLAN_HANDOFF_VNEXT_REINCARNATION.md` and `aim-agy_os/planning-artifacts/PLAN_HANDOFF_VNEXT_REINCARNATION.md` (aim-grok).

### Acceptance (short)

1. `./aim handoff` → `continuity/CURRENT.md` + `handoff_result.json`; cannot false-SUCCESS.
2. Nightly job: FR cleaned session → wiki under correct root → embed cleaned FR → DB rows.
3. Nightly blackbox seals all sessions; failures do not block A or B.
4. Agents consume packets/wiki; must not invent pipelines or open tickets without result JSON.

### Labels / tracking

- Type: **epic / architecture**
- Replaces / supersedes entangled reincarnate+wiki+vault single-path design
- Implementation: feature-flag side-by-side (`AIM_HANDOFF_V2`), then cutover

---

## Full plan (entire document)
