...
- **The Spawn:** It creates a brand-new, clean `tmux` session (`agy_reincarnation_[TIMESTAMP]` on aim-agy; prefix follows `AIM_VESSEL_CLI`, e.g. `grok_` / `opencode_` on sibling vessels).
