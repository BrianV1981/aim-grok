...
5.  **The Wiki Agent (optional):** When `AIM_WIKI_MODE=agent`, it spawns an ephemeral tmux session named `{vessel}_wiki_{project_slug}_{timestamp}` (not a shared `wiki_agent` / `scribe_agent_aim` singleton). Default mode is **deterministic** (no second agent). The agent (if used) weaves `_ingest/` into the wiki, logs to `log.md`, re-embeds into `memory_lance`, and exits.
